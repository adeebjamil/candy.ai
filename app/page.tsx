  ...
  return (
    <div className="flex flex-col flex-1 pb-24 bg-white">
      ...
      <section className="w-full bg-surface py-20 px-8 relative overflow-hidden">
        ...
        <div className="absolute -bottom-24 -left-24 w-96 h-96 bg-primary/10 rounded-full blur-3xl animate-blob"></div>
        <div className="absolute top-10 right-10 w-64 h-64 bg-secondary/10 rounded-full blur-3xl animate-blob animation-delay-2000"></div>
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="max-w-3xl space-y-8">
            <div className="inline-flex items-center gap-2 bg-white px-4 py-2 rounded-2xl shadow-sm border border-border">
              ...
              <span className="text-[14px] font-bold text-primary tracking-wide uppercase">✨ Experience True Connection</span>
            </div>
            ...
            <p className="text-xl md:text-2xl text-text-muted max-w-2xl font-medium leading-relaxed">
              Step into a world of endless possibilities. Create your perfect AI partner, share deep connections, and build unforgettable memories together. 💖
            </p>
            
            <div className="pt-4 flex flex-wrap gap-5">
              <Link href="/create" className="bg-primary hover:bg-primary-hover text-white px-10 py-5 rounded-[2rem] font-bold text-xl transition-all hover:scale-105 shadow-xl shadow-pink-200 flex items-center gap-3">
                <Plus className="w-6 h-6" />
                Craft Your AI
              </Link>
              <Link href="/discover" className="bg-white hover:bg-surface text-foreground px-10 py-5 rounded-[2rem] font-bold text-xl transition-all shadow-sm border border-border flex items-center gap-3">
                Explore Gallery
              </Link>
            </div>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-8 py-20 w-full">
        <div className="flex justify-between items-end mb-12">
          <div className="space-y-2">
            <h2 className="text-4xl font-black text-foreground tracking-tight">Trending Characters</h2>
            <p className="text-text-muted font-semibold">The most popular companions this week</p>
          </div>
          <Link href="/discover" className="text-primary font-bold hover:underline">View All</Link>
        </div>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {/* Create Own Character Card */}
          <Link href="/create" className="group rounded-[2.5rem] overflow-hidden relative aspect-[3/4.5] bg-surface border-2 border-dashed border-border flex flex-col justify-center items-center text-center p-8 hover:bg-white hover:border-primary/50 transition-all duration-500">
            <div className="w-20 h-20 rounded-full bg-white shadow-xl flex items-center justify-center mb-6 group-hover:scale-110 group-hover:rotate-90 transition-transform duration-500">
              <Plus className="w-10 h-10 text-primary" />
            </div>
            <h3 className="font-black text-foreground text-2xl">Design Yours</h3>
            <p className="text-primary font-bold mt-2">Start Creating ✨</p>
          </Link>
            ...
            <Link key={char.id} href={`/ai-girlfriend/${char.name.toLowerCase().replace(" ", "-")}`} className="group rounded-[2.5rem] overflow-hidden relative aspect-[3/4.5] bg-gray-100 shadow-sm hover:shadow-2xl transition-all duration-500 hover:-translate-y-2">
              ...
              <div className="absolute bottom-0 left-0 w-full p-8 transform transition-transform duration-500 group-hover:-translate-y-2">
                ...
                <div className="mt-4 flex gap-2 overflow-hidden opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                  ...
                  <span className="bg-white/20 backdrop-blur-md text-white text-[10px] font-bold px-3 py-1 rounded-full border border-white/30 whitespace-nowrap">Caring</span>
                </div>
              </div>
            </Link>
          ))}
      ...
      <section className="max-w-4xl mx-auto px-8 py-20 w-full">
        <div className="text-center mb-16 space-y-4">
          <h2 className="text-5xl font-black tracking-tight">Common Questions</h2>
          <p className="text-xl text-text-muted font-medium">Everything you need to know about CandyAI</p>
        </div>
        <div className="grid gap-4">
          {[
            ...
            <div key={i} className="group bg-surface p-8 rounded-[2rem] border border-transparent hover:border-primary/20 hover:bg-white hover:shadow-xl hover:shadow-pink-50 transition-all duration-300 cursor-pointer">
              <h3 className="font-bold text-xl text-foreground flex justify-between items-center">
                {q}
                <div className="bg-white p-2 rounded-xl text-primary shadow-sm group-hover:bg-primary group-hover:text-white transition-all duration-300">
                  <Plus className="w-6 h-6" />
                </div>
              </h3>
            </div>
